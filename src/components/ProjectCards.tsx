              ...
              <div className="flex items-center gap-1">
                ...
                {p.sources.map((src) => {
                  ...
                  return (
                    <span
                      key={src}
                      title={`View ${S.label} evidence`}
                      onClick={(e) => {
                        e.stopPropagation();
                        navigate(`/sources?source=${src === "meeting" ? "transcript" : src}&project=${p.slug}`);
                      }}
                      className="w-5 h-5 rounded bg-secondary flex items-center justify-center hover:bg-accent/20 hover:scale-110 transition-all cursor-pointer"
                    >
